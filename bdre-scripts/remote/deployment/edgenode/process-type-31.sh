...
#copy the hqls for source stage load and repair table
cp -r $BDRE_HOME/bdre-scripts/hql/* $BDRE_APPS_HOME/$busDomainId/$processTypeId/$processId/hql

#copy required jars for cluster-migration
cp -f $BDRE_HOME/lib/cluster-migration/cluster-migration-*-jar-with-dependencies.jar $BDRE_APPS_HOME/$busDomainId/$processTypeId/$processId/lib
if [ $? -eq 1 ]
then exit 1
fi
